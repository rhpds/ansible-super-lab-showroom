...
** Content from xref:22-content-build.adoc[Lab 2.2 Build a custom Collection and Execution Environment]
...
. Create a new playbook in a file named `test_pah_ee.yml` in the `/projects/lab_playbooks` directory. This playbook uses the `debug` module to print a message that has been formatted by our custom `cowsay` filter:
+
...
----
+
NOTE: Be sure to enter your Gitea username and password in the dialog since this is a new repository.
+
. Verify the files appear in Gitea
+
image::23-content-execute/gitea-lab-playbooks.png[Lab Playbooks,link=self,window=blank]
...
+
image::23-content-execute/automation-controller-credentials-overview.png[Navigate to Credentials,link=self,window=blank]
+
. Click **Create credential** to add a new credential.
+
image::23-content-execute/automation-controller-create-credential.png[Create Credentials,link=self,window=blank]
+
...
image::23-content-execute/automation-controller-create-credential-dialog.png[Create Credentials Dialog,link=self,window=blank]
...
+
image::23-content-execute/automation-controller-create-credential-2.png[Create Credentials,link=self,window=blank]
+
. Enter the following details on the _Create Credential_ page:
.. *Name*: `Gitea`
...
. Click **Create credential** to save the new credential.
+
image::23-content-execute/automation-controller-create-credential-git-dialog.png[Create Credentials Dialog,link=self,window=blank]
...
+
image::23-content-execute/automation-controller-ee-overview.png[Navigate to Execution Environments,link=self,window=blank]
+
. Click **Create execution environment** to add a new execution environment.
+
image::23-content-execute/automation-controller-create-ee.png[Create Execution Environment,link=self,window=blank]
+
...
image::23-content-execute/automation-controller-create-ee-dialog.png[Create Execution Environment Dialog,link=self,window=blank]
...
. From the Credentials page in Automation Controller, click **Create credential** once again to add a new credential.
+
image::23-content-execute/automation-controller-create-credential-3.png[Create Credentials,link=self,window=blank]
+
...
. Click **Create credential** to save the new credential.
+
image::23-content-execute/automation-controller-create-credential-pah-dialog.png[Create Credentials Dialog,link=self,window=blank]
...
+
image::23-content-execute/automation-controller-organizations-overview.png[Navigate to Organizations,link=self,window=blank]
+
. Next to the _Default_ organization, click the pencil icon to edit the organization.
+
image::23-content-execute/automation-controller-organizations-edit.png[Edit Organization,link=self,window=blank]
+
...
+
image::23-content-execute/automation-controller-organizations-add-credentials.png[Add Credentials to Organization,link=self,window=blank]
+
...
+
image::23-content-execute/automation-controller-projects-overview.png[Navigate to Projects,link=self,window=blank]
+
. Click **Create project** to add a new project.
+
image::23-content-execute/automation-controller-create-project.png[Create Project,link=self,window=blank]
+
...
+
image::23-content-execute/automation-controller-create-project-dialog.png[Create Project Dialog,link=self,window=blank]
+
. Confirm that the project has been synced successfully by looking _Success_ underneath _Last job status_.
+
image::23-content-execute/automation-controller-sync-project.png[Project Sync Status,link=self,window=blank]

=== 2.8: Create a Job Template
...
+
image::23-content-execute/automation-controller-templates-overview.png[Navigate to Templates,link=self,window=blank]
+
. Click the **Create template** dropdown and select **Create job template**.
+
image::23-content-execute/automation-controller-create-job-template.png[Create Job Template,link=self,window=blank]
+
...
+
image::23-content-execute/automation-controller-create-job-template-dialog.png[Create Job Template Dialog,link=self,window=blank]

=== 2.9: Launch the Job Template and Verify
...
+
image::23-content-execute/automation-controller-launch-job-template.png[Launch Job Template,link=self,window=blank]
+
...
+
image::23-content-execute/automation-controller-job-template-success.png[Job Template Success,link=self,window=blank]

=== 2.10: Add Job Template for Windows Node
...
----
+
image::23-content-execute/jt-provision-windows.png[Job Template Success,link=self,window=blank]

== 3: Collections Management

The final exercise in this lab is to customize the content sources from link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.7/administer-assembly_working_with_namespaces#proc-delete-namespace_results[Remote Automation Hub / Ansible Galaxy instances,window=_blank] into Private Automation Hub. To support lab exercises later in this bootcamp, we will specify additional collections to be synchronized.

=== 3.1: Update Community Remote

. From the navigation menu on the left, expand **Automation Content** and click on **Remotes**.
+
image::23-content-execute/remotes-overview.png[Remotes Navigation,link=self,window=blank]
+
. Click the pencil icon to edit the `community` remote.
+
image::23-content-execute/remotes-edit-community.png[Edit Community Remote,link=self,window=blank]
+
...
+
image::23-content-execute/remotes-create-remote.png[Create Remote,link=self,window=blank]
+
...
image::23-content-execute/remotes-add-remote.png[Add Remote,link=self,window=blank]
...
+
image::23-content-execute/repositories-overview.png[Add Remote,link=self,window=blank]
+
. Edit the _validated_ repository by selecting the pencil icon
+
image::23-content-execute/repositories-edit-validated.png[Edit Validated Repository,link=self,window=blank]
+
. Update the `Remote` field by selecting `validated` from the drop down.
. Click **Save repository** to update the repository.

image::23-content-execute/repositories-update-validated.png[Updated Validated Repository,link=self,window=blank]

=== 3.2: Sync Repositories

Finally, synchronize  *both* the `validated` and `community` repositories to pull in the new collections within the _Repositories_ page.
...
+
image::23-content-execute/sync-validated-repository.png[Sync Validated Repository,link=self,window=blank]
+
...
. https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.7/administer-manage_your_organization_s_automation_content[Private Automation Hub Documentation^]
