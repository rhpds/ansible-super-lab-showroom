= Lab: Building and Managing Content
...
. First, create a new directory named `env` in the `/projects` directory to hold various scripts and variables. 
+
[source,bash,role=execute,subs="verbatim,attributes,quotes"]
----
mkdir /projects/env
----
. Now, create a file called `set_pah_vars.env` in your `/projects/env/` directory with the following contents. Be sure to use your PAH API token created previously:
+
