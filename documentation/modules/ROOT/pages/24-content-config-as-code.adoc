...
Create a new `execution-environment.yml` file within the `aap-as-code` directory with the following content:
...
NOTE: Ensure that the `PAH_API_TOKEN` environment variable is set within your terminal session so that the build process can retrieve the required set of dependencies. You may need to run: `source /projects/env/set_pah_vars.env`
...
ansible-builder build \
  ...
  --verbosity 2<1>
----
<1> `--verbosity 3` will produce an enormous amount of output but is very useful for viewing all the steps involved.
+
...
Create a separate secrets file to avoid putting any secrets directly in playbooks.
...
NOTE: The more components installed at once, the longer it will take. Feel free to start with *controller* only and re-run with more components enabled to iteratively deploy additional components, similar to the lab in Module 1.

== 3: Install AAP on OpenShift via Infrastructure as Code

=== 3.1: Run Installation Playbook

Now that the required environment is setup and Execution Environment created, we can test it locally before pushing it to PAH.
...
Config as Code leverages standard Ansible variables files containing properties that will be used to drive the configuration of AAP. These variables include hosts, usernames, and other items that are reused in each role contained within the Config as Code collection.

=== 4.1: Define Secrets

Best practice is to use `ansible-vault` to encrypt files. For this lab we will keep our secrets in the same file we have been using for simplicity.
...
Since we already have a few AAP deployments on OpenShift, we can consider different namespaces each a different AAP enironment, with differnet URLs, secrets and resources for each deployment. For now let's create one for this environment.

. Create a new variable file to represent this environment. This would typipically be handled by inventory `group_vars` or `vars_files`, but for this lab we'll simplify to a single file. Place this file in the same directory `/projects/aap-as-code/`:
+
...
Create a simple playbook to call the `inra.aap_configuration.dispatch` role which will evaluate the variables defined and call the appropriate roles in the proper order basedon precedence.
...
=== 4.6: Troubleshoot Failed Runs

The first time this playbook runs it will fail. By default you cannot see why because of secure logging. If you read the role readme for the failed task you will see a section on how to disable secure logging. Add these two variables to the `lab.yml` environment file:
...
. Append the following variables anywhere in the `lab.yml` file:
+
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/aap-as-code/lab.yml"]
----
...
...
----
+
. Now run the playbok again and this time scroll up and look more closely at the output:
+
[source,output,subs="verbatim,attributes,callouts"]
----
TASK [infra.aap_configuration.collect_async_status : Create/Update Credential Windows | Wait for finish the credential creation] ***
...
=== 5.3 Push Execution Environment

[source,bash,role=execute,subs="verbatim,attributes"]
----
podman push \
  aap-aap.{openshift_cluster_ingress_domain}/config_as_code_ee:1.0
----
