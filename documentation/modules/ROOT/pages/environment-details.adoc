...
[cols="1,1"]
|===
|Hostname (isolated, can only be reached from AAP and Dev Spaces)
|windows.aap.svc.cluster.local
...
|===
|API Endpoint
|link:{litellm_api_base_url}[{litellm_api_base_url},window=_blank]
...
|===
|LibreChat URL
|link:https://librechat-librechat.{openshift_cluster_ingress_domain}[https://librechat-librechat.{openshift_cluster_ingress_domain},window=_blank]
