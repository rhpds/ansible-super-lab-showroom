...
** Content from xref:31-events-eda-overview-active-monitoring.adoc[Lab 3.1 Event-Driven Ansible Overview and Active Health Monitoring]
...
. In your `lab_playbooks` repository, create the `check_kafka.yml` rulebook in the `rulebooks` directory:
+
...
+
image::32-events-automated-reactive-remediation/eda-projects-sync.png[Sync EDA Project,link=self,window=blank]
...
+
image::32-events-automated-reactive-remediation/eda-ra-create.png[Create Rulebook Activation,link=self,window=blank]
...
+
image::32-events-automated-reactive-remediation/eda-ra-disable-check-kafka.png[Confirm Disable EDA Rulebook Activation,link=self,window=blank]
