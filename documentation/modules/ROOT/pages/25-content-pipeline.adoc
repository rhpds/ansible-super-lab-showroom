...
[.sidebar]
****
* **Before you start:** Access to Dev Spaces and AAP in xref:environment-details.adoc[Environment Details], content from xref:22-content-execute.adoc[Lab 2-3. Develop a Playbook to execute it all]
* **Prepares you for:** Using OpenShift Pipelines to automate the collection build and publish process
* **Lab Type:** Self-Contained (_You can skip this without missing steps for other labs_)
...
Create another Gitea repository, separate from the _red_hat_one_super_lab_ repository that was xref:21-content-build.adoc[created previously], which will serve as a working CI/CD pipeline development repository.
...
+
image::23-pipelines/pipelines-gitea-repo-new-select.png[Select New Gitea Repository]
+
...
image::23-pipelines/pipelines-gitea-repo-new.png[New Gitea Repository]
...
image::23-pipelines/pipelines-clustertriggerbindings.png[Cluster Trigger Bindings]
...
image:23-pipelines/pipelines-pipeline.png[Ansible Collection CI Pipeline]
...
image:23-pipelines/pipelines-trigger-config.png[Pipeline Trigger Config]
...
image:23-pipelines/pipelines-webhook-url.png[Pipelines Webhook URL]
...
image:23-pipelines/pipelines-gitea-webhook-config.png[Webhook Configuration]
...
image:23-pipelines/pipelines-gitea-webhook-test.png[Test Webhook Delivery]
...
image:23-pipelines/pipelines-devspace-role-create.png[Create dad_joke Role]
...
image::23-pipelines/pipelines-pah-verify.png[Verify Collection in Automation Hub]
...
Ensure that you have the necessary environment variables set to connect to your Private Automation Hub instance as was configured in xref:21-content-build.adoc[Building Ansible Content Collections]. These variables can be set by running the following command in your terminal.
