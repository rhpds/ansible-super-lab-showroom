...
collections:
  ...
  - infra.aap_utilities
----
. Go to *Automation Content* -> *Repositories* and sync both the *rh-certified* and *validated* repositories
...
images:
  base_image:
    name: aap-aap.{openshift_cluster_ingress_domain}/ansible-automation-platform-27/ee-minimal-rhel9:2.16
...
additional_build_steps:
  prepend_galaxy:
    ...
    - ENV ANSIBLE_GALAXY_SERVER_CERTIFIED_URL='https://aap-aap.apps.cluster-b6g82.dyn.redhatworkshops.io/pulp_ansible/galaxy/rh-certified/'
    - ENV ANSIBLE_GALAXY_SERVER_CERTIFIED_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_VALIDATED_URL='https://aap-aap.apps.cluster-b6g82.dyn.redhatworkshops.io/pulp_ansible/galaxy/validated/'
    - ENV ANSIBLE_GALAXY_SERVER_VALIDATED_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_COMMUNITY_URL='https://aap-aap.apps.cluster-b6g82.dyn.redhatworkshops.io/pulp_ansible/galaxy/community/'
    - ENV ANSIBLE_GALAXY_SERVER_COMMUNITY_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_PUBLISHED_URL='https://aap-aap.apps.cluster-b6g82.dyn.redhatworkshops.io/pulp_ansible/galaxy/published/'
...
+
image::25-content-config-as-code/installed-operators-aap.png[Installed Operators in the AAP Project,link=self,window=blank]
+
. Navigate to *Networking* -> *Routes*
. Open the URL under *Location* for the *lab* instance in a new window which should be link:https://lab-aap-as-code.{openshift_cluster_ingress_domain}/[https://lab-aap-as-code.{openshift_cluster_ingress_domain}/^]
...
. Activate Ansible Automation Platform using the steps as defied in xref:13-platform-aap-on-openshift.adoc[1.3 Ansible Automation Platform  on OpenShift] 

== 4: Configuration As Code

Config as Code uses standard Ansible variable files containing properties that will be used to drive the configuration of AAP. These variables include all the typical objects usually configured manually via ClickOps, organizations, inventories, hosts, projects, and other items that each have a role contained within the Config as Code collection that will configure these through modules.
...
----
<1> Obtain this random password from *OpenShift* -> *Workloads* -> *Secrets* -> lab-admin-password. Click the 'copy to clipboard' icon next to the `password` field and place in this property within the secrets file.
...
. Create a new variable file called `lab.yml` to represent this environment. This would typically be handled by inventory `group_vars` or separate `vars_files`, but for this lab we'll simplify to a single file. Place this file in the same directory `/projects/aap-as-code/`:
+
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/aap-as-code/lab.yml"]
----
---
aap_hostname: 'lab-aap-as-code.{openshift_cluster_ingress_domain}'
aap_validate_certs: false
----

=== 4.3: Define some Configuration

Define a few Ansible Automation Platform objects as specified below in the data structure to configure the platform. This will create a new organization called `config_as_code`, an inventory with our Windows host called `super-lab` and the Credentials to access the instance. Append the following to the environment variable file (`lab.yml`) created in the previous step:
...
Create a simple playbook called `configure_aap.yml` to invoke the `inra.aap_configuration.dispatch` role which will evaluate the variables defined and call the appropriate roles in the proper order based on precedence.
...
Verify what did work by launching Ansible Automation Platform in the `aap-as-code` project.
...
+
. Fix the issue by adding an `organization` definition to the credential as shown below:
+
...
== 6. Remove the Ansible Automation Platform Instance

After you have reviewed the results in the prior section, you can remove the Ansible Automation Platform instance from OpenShift in order to conserve resources in the environment. Utilize the steps as described xref:13-platform-aap-on-openshift.adoc#uninstall-aap-instance[here] to remove the instance.

== 7: Publish Artifacts

With the proper configurations in place, we can save our work to our Gitea instance.

=== 7.1: Create Gitea Repository
...
Create a file named `.gitignore` in the `/projects/aap-as-code` directory with the following content to exclude unnecessary files and directories from being tracked by Git.
...
Enter the credentials when prompted to authenticate with Gitea within your OpenShift Dev Spaces window.

=== 7.3: Push code to repository
...
. https://console.redhat.com/ansible/automation-hub/repo/validated/infra/aap_configuration/content/role/gateway_organizations/[gateway_organizations role^]
. https://console.redhat.com/ansible/automation-hub/repo/validated/infra/aap_configuration/content/role/controller_teams/[controller_teams role^]
