...
image::21-content-dev-ee/api-token-copy.png[Copy the API Token]
...
. First, create a file in your `/home/user/` directory with the following contents, be sure to use your PAH API token created previously:
+
NOTE: Replace `{your_api_token}` with the personal API token you just copied from PAH.
+
[source,bash,role=execute,subs="verbatim,attributes",title="/home/user/pah_vars.env"]
----
...
+
[source,bash,role=execute,subs="verbatim,attributes",title=""]
----
dependencies: {}
----
+
...
+
image::21-content-dev-ee/namespaces-overview.png[Namespaces overview]
+
. Enter `red_hat_one` for the name. The rest of the values can be left blank
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
ansible-galaxy collection publish --server {aap_controller_web_url}/api/galaxy/ red_hat_one-super_lab-1.0.0.tar.gz --token ${PAH_API_TOKEN}
----
+
. Output:
+
[source,bash]
----
Publishing collection artifact '/projects/lab_collection/red_hat_one-super_lab-1.0.0.tar.gz' to cmd_arg https://aap-aap.apps.cluster-5tt52.dynamic.redhatworkshops.io/api/galaxy/
Collection has been published to the Galaxy server cmd_arg https://aap-aap.apps.cluster-5tt52.dynamic.redhatworkshops.io/api/galaxy/
Waiting until Galaxy import task https://aap-aap.apps.cluster-5tt52.dynamic.redhatworkshops.io/api/galaxy/v3/imports/collections/019bbddb-f6e4-7613-9975-45dec02119c3/ has completed
Collection has been successfully published and imported to the Galaxy server cmd_arg https://aap-aap.apps.cluster-5tt52.dynamic.redhatworkshops.io/api/galaxy/
----
+
...
+
image::21-content-dev-ee/collection-approvals-overview.png[Collection Approvals overview]
+
. The `red_hat_one.super_lab` should be listed as pending approval. Click the **Thumbs Up** icon to approve and sign the collection.
+
...
+
image::21-content-dev-ee/navigate-collections.png[Navigate to Collections]
+
...
+
image::21-content-dev-ee/remote-registries-overview.png[Navigate to Remote Registries]
+
...
+
image::21-content-dev-ee/execution-environments-navigate.png[Navigate to Execution Environments]
+
...
Back in your Dev Spaces instance, update the `.gitignore` file within the `my_pah_project` directory containing the `super_lab` collection and append the following lines to exclude files that should not be committed to the repository.
...
git remote add origin {gitea_console_url}/{gitea_user}/red_hat_one_super_lab.git
...
ansible-galaxy collection build
ansible-galaxy collection publish -s {aap_controller_web_url}/api/galaxy/ red_hat_one-super_lab-1.0.1.tar.gz --token $PAH_API_TOKEN
----
...
Now that both the collection and Execution Environment has been created and published to Private Automation hub, we'll create a playbook to use the filter contained within the `red_hat_one.super_lab` collection.
