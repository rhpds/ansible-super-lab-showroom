...
+
[source,bash,role=execute,subs="verbatim,attributes,quotes",title="/home/user/set_pah_vars.env"]
----
...
+
[source,bash,role=execute,title="/home/user/unset_pah_vars.env"]
----
...
----
source /home/user/set_pah_vars.env
----
+
...
+
. Click *Save remote*
. Go to Automation Content -> Repositories
. Next to *rh-certified* click the kebob (3 dots) and select *Sync repository*
...
----
<1> This is the URL copied in a previous step.
...
----
+
NOTE: The `${PAH_API_TOKEN}` environment variable was set previously containing the value of the API Token from PAH. Set this variable once again if your terminal session has been restarted via the `/home/user/set_pah_vars.env` file.
+
[source,output,subs="verbatim,quotes"]
----
Running command:
  podman build -f context/Containerfile -t my-pah-ee:1.0 --build-arg=TOKEN=*_removed_*<1> context
Complete! The build context can be found at: /projects/lab_collection/context
----
<1> Token is passed from the CLI and used in the build process
+
