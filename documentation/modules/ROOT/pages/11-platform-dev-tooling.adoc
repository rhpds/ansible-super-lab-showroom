...
[source,bash,role=execute,subs="verbatim,attributes"]
----
chmod +x /projects/env/config.sh
----
...
. Check what branch you are in. Notice is it also dynamically displayed in your prompt:
+
...
----
+
. Create a new branch called `test`, which is a copy of `main`:
+
[source,role=execute,bash,subs="verbatim,attributes"]
----
git checkout -b test
