= Module 2 Lab 4: Content - Ansible Test-Driven Development with Molecule
...
* **Prepares you for:** Using Molecule for Test Driven Development
...
First, launch your OpenShift Dev Spaces workspace and create the `db_server` role resource within the `red_hat_one.super_lab` collection in the `/projects/lab_collection` directory.
...
Modify the files inside `/projects/lab_collection/roles/db_server/` directory to configure PostgreSQL.

First, update the `/projects/lab_collection/roles/db_server/defaults/main.yml` file with the following content:

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/roles/db_server/defaults/main.yml"]
----
---
# defaults file for red_hat_one.super_lab.db_server
...
Next replace the entire contents of the main task file `/projects/lab_collection/roles/db_server/tasks/main.yml`. Notice there are no installation, initialization, or service management tasks specified.

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/roles/db_server/tasks/main.yml"]
----
---
# tasks file for red_hat_one.super_lab.db_server
...
Update the argument specification file for the db_server role in `/projects/lab_collection/roles/db_server/meta/argument_specs.yml`:

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/roles/db_server/meta/argument_specs.yml"]
----
---
# argument spec file for red_hat_one.super_lab.db_server
argument_specs:
  main:
...
Now, to enable reusability we can move the generated playbooks into a shared directory called `utils` which eliminates some of the boilerplate content and redundancy. Move the Molecule playbooks into the `utils` directory for sharing between scenarios.

[source,bash,role=execute,subs="verbatim,attributes"]
----
mv molecule/db_server/{converge.yml,create.yml,destroy.yml,verify.yml} molecule/utils/playbooks/
----
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
rm -rf molecule/integration_hello_world
----

The primary logic behind Molecule execution is defined in the link:https://ansible.readthedocs.io/projects/molecule/configuration/[`molecule.yml`,window=_blank] configuration file. Replace the contents of `/projects/lab_collection/extensions/molecule/db_server/molecule.yml` with the following configuration:

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/db_server/molecule.yml"]
----
---
dependency:
  name: galaxy
...
Create a new `requirements.yml` file in the `/projects/lab_collection/extensions/molecule/db_server/` directory to specify the required Ansible collections for this scenario:

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/db_server/requirements.yml"]
----
---
collections:
  - containers.podman
...
The scenario uses shared playbook files for container management and testing. Update the following files in the `/projects/lab_collection/extensions/molecule/utils/playbooks/` directory:
...
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/create.yml"]
----
---
...
Create a new directory called `tasks` inside `/projects/lab_collection/extensions/molecule/utils/playbooks/` and create a new file called `create-fail.yml` playbook in the newly created directory. This playbook handles failure scenarios during container creation by retrieving and displaying container logs for debugging.

[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/tasks/create-fail.yml"]
----
---
...
The `prepare.yml` playbook handles any pre-testing setup tasks. This optional phase in Molecule allows you to configure the test environment before applying your Ansible role, such as installing dependencies or setting up prerequisites. Create a new file named `prepare.yml` file in the `/projects/lab_collection/extensions/molecule/utils/playbooks/` directory with the following content:

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/prepare.yml"]
----
---
- name: Prepare play
  hosts: molecule
...
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/converge.yml"]
----
---
- name: Converge
  hosts: all
  tasks:
    ...
    - name: "Include the db_server role"
      ansible.builtin.include_role:
        name: "red_hat_one.super_lab.db_server"
...
...
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/verify.yml"]
----
---
- name: Verify
  hosts: all
...
The `cleanup.yml` playbook handles cleanup of temporary files and artifacts created during testing, helping maintain a clean test environment between test runs without destroying the actual test infrastructure. Create a new file called `cleanup.yml` in the `/projects/lab_collection/extensions/molecule/utils/playbooks/` directory with the following content:

[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/cleanup.yml"]
----
---
...
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/destroy.yml"]
----
---
...
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/utils/playbooks/noop.yml"]
----
---
- name: No-op
  hosts: localhost
...
As you may have noticed, minimal changes were required in the `noop.yml` file.
...
Update the `galaxy.yml` file in the collection directory (`/projects/lab_collection/`) to add a dependency on `"community.postgresql": "*"` and increment the version number to `1.0.2` as shown below:
...
Add the dependency under the `dependencies` section so the file looks like the following:
...
ansible-galaxy collection install red_hat_one-super_lab-1.0.2.tar.gz --force
ansible-galaxy collection publish -s {aap_controller_web_url}/api/galaxy/ red_hat_one-super_lab-1.0.2.tar.gz --token $PAH_API_TOKEN
----
