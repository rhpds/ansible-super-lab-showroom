= Module 2 Lab 6: Content - Creating a CI/CD Pipeline for Collections
...
* **Prepares you for:** Using OpenShift Pipelines to automate the collection build and publish process
...
. Repository for the `red_hat_one.super_lab` collection has been created on Gitea
. You have access to your Dev Spaces workspace for your environment
. Make sure the Gitea repository is *public*

NOTE: The lab setup should have been completed in the prior modules.
...
Create another Gitea repository, separate from the _red_hat_one_super_lab_ repository that was xref:22-content-build.adoc[created previously], which will serve as a working CI/CD pipeline development repository.
...
+
image::26-content-pipeline/pipelines-gitea-repo-new-select.png[Select New Gitea Repository,link=self,window=blank]
+
. On the New Repository page, enter **red_hat_one_ci_cd_pipeline** in the _Repository Name_ field.
. Leave everything else as their default values and click on the button at the bottom, **Create Repository**.

image::26-content-pipeline/pipelines-gitea-repo-new.png[New Gitea Repository,link=self,window=blank]
...
. First change to the `/projects/` directory:
+
...
----
+
. Clone the _red_hat_one_ci_cd_pipeline_ repository you just created into your workspace
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
git clone {gitea_console_url}/{gitea_user}/red_hat_one_ci_cd_pipeline.git
----
+
[source,output]
----
Cloning into 'red_hat_one_ci_cd_pipeline'...
warning: You appear to have cloned an empty repository.
----

NOTE: You can safely ignore the `warning: You appear to have cloned an empty repository.` message as you will populate the repository with content in the next steps.
...
In the _red_hat_one_ci_cd_pipeline_ directory, create a new file called `collection-pipeline.yml` with the following content.

.collection-pipeline.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/red_hat_one_ci_cd_pipeline/collection-pipeline.yml"]
----
...
Within the _red_hat_one_ci_cd_pipeline_ directory, create a new file called `collection-cluster-trigger-binding.yml` with the following content.

.collection-cluster-trigger-binding.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/red_hat_one_ci_cd_pipeline/collection-cluster-trigger-binding.yml"]
----
...
In the Dev Spaces terminal, change into the _red_hat_one_ci_cd_pipeline_ directory and use the OpenShift CLI to apply the configurations to the OpenShift cluster.

[source,bash,role=execute,subs="verbatim,attributes"]
----
cd /projects/red_hat_one_ci_cd_pipeline
...
image::26-content-pipeline/pipelines-clustertriggerbindings.png[Cluster Trigger Bindings,link=self,window=blank]
...
image::26-content-pipeline/pipelines-pipeline.png[Ansible Collection CI Pipeline,link=self,window=blank]
...
Within the _red_hat_one_ci_cd_pipeline_ directory, create an Ansible playbook in the file `collection-publish.yml` with the following content.

.collection-publish.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/red_hat_one_ci_cd_pipeline/collection-publish.yml"]
----
---
...
Commit all files you have created in the _red_hat_one_ci_cd_pipeline_ directory and push the contents to the Gitea repository
...
.. playbook-repo: `{gitea_console_url}/{gitea_user}/red_hat_one_ci_cd_pipeline.git`
.. collection-repo-version: `$(tt.params.git-revision)`
.. shared-workspace: `VolumeClaimTemplate`
. Click **Add** to create the Trigger.

image::26-content-pipeline/pipelines-trigger-config.png[Pipeline Trigger Config,link=self,window=blank]
...
image::26-content-pipeline/pipelines-webhook-url.png[Pipelines Webhook URL,link=self,window=blank]
...
. Navigate to the link:{gitea_console_url}/{gitea_user}/red_hat_one_super_lab[Gitea red_hat_one_super_lab Repository,window=_blank]
...
. Click the green *Add Webhook* button on the right side of the window and select *Gitea* from the dropdown selections
...
image::26-content-pipeline/pipelines-gitea-webhook-config.png[Webhook Configuration,link=self,window=blank]

==== 4.4.4: Test Webhook

With the webhook created, let's send a test payload to confirm that it is working properly.

. From the link:{gitea_console_url}/{gitea_user}/red_hat_one_super_lab/settings/hooks[Webhooks page,window=_blank] of the red_hat_one_super_lab repository in Gitea, click on the link for the webhook you just created
. At the bottom of the page, click the *Test Delivery* button to trigger the pipeline

image::26-content-pipeline/pipelines-gitea-webhook-test.png[Test Webhook Delivery,link=self,window=blank]

A _Green_ check mark next to the delivery indicates that the webhook was successfully sent to OpenShift.

NOTE: You can see the status of the pipeline by going back into your OpenShift console and navigating to the pipeline you created and clicking on _PipelineRuns_. The _PipelineRun_ will fail as there are parameters that are not populated when testing the webhook. As long as the pipeline is triggered successfully, you can proceed to the next section.

=== 4.5: Update and Push New Version of Ansible Collection to Gitea

Update the Ansible collection created previously and contained in the `/projects/lab_collection` directory by adding a new role called `dad_joke` that fetches and displays a random dad joke from the link:https://icanhazdadjoke.com[icanhazdadjoke,window=_blank] API. In addition, we will add a Molecule test scenario to validate the functionality of the new role.
...
. Click on `Role`
. Provide the path for your collection root directory (`/projects/lab_collection`)
. Name the role `dad_joke`
. Click **Create** to create the role

image::26-content-pipeline/pipelines-devspace-role-create.png[Create dad_joke Role,link=self,window=blank]
...
Click on the file explorer and expand the `/projects/lab_collection/roles/dad_joke/tasks` directory within the collection. Place the following content within the `/projects/lab_collection/roles/dad_joke/tasks/main.yml` file 

.main.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/roles/dad_joke/tasks/main.yml"]
----
---
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
cd /projects/lab_collection/extensions
----
+
...
----
+
. Replace the contents of the `/projects/lab_collection/extensions/molecule/dad_joke/converge.yml` file with the contents below
+
.converge.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/dad_joke/converge.yml"]
----
---
  ...
  tasks:
    - name: "Include the dad_joke role"
      ansible.builtin.include_role:
        name: red_hat_one.super_lab.dad_joke
...

----   
+
. Replace the contents of the `/projects/lab_collection/extensions/molecule/dad_joke/molecule.yml` file with the contents below
+
.molecule.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_collection/extensions/molecule/dad_joke/molecule.yml"]
----
---
...
Confirm the PipelineRun completes successfully before proceeding to the next section. It is possible that one of the steps has failed.

NOTE: This run will fail if there are any lint errors found. We intentionally introduced some in earlier labs along the way. View the PipelineRun output to determine where the errors are and fix them.
...
image::26-content-pipeline/pipelines-pah-verify.png[Verify Collection in Automation Hub,link=self,window=blank]

=== 4.7: Install and Use the updated collection from PAH
...
Ensure that you have the necessary environment variables set to connect to your Private Automation Hub instance as was configured in xref:22-content-build.adoc[Module 2 Lab 2 Building Ansible Content Collections]. These variables can be set by running the following command in your terminal.
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
ansible-galaxy collection install --force red_hat_one.super_lab
----
+
. Create the following ansible playbook in a file named `test_dad_joke.yml` in the _lab_playbooks_ repository to test the new collection in the `playbooks` directory of the collection.
+
./projects/lab_playbooks/test_dad_joke.yml
[source,yaml,role=execute,subs="verbatim,attributes",title="/projects/lab_playbooks/test_dad_joke.yml"]
----
---
- name: Random Dad Joke Generator
  ...
  roles:
    - red_hat_one.super_lab.dad_joke
...

----   
+
. From the root of the _lab_playbooks_ repository, execute the playbook and if successful, it should return a Dad Joke
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
cd /projects/lab_playbooks
ansible-playbook test_dad_joke.yml
----
