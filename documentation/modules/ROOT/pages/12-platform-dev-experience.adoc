...
** Completion of xref:11-platform-dev-spaces.adoc[Lab 1.1 OpenShift Dev Spaces for Ansible Development]
* **Prepares you for:** Using git effectively, pre-commit, gitleaks and Ansible Development Tools
* **Lab Type:** Self-Contained (_You can skip lab this without missing steps for other labs_)
...
. This lab takes place in link:https://devspaces.{openshift_cluster_ingress_domain}[OpenShift Dev Spaces,window=_blank]
...
. Source these files and watch how your prompt changes:
+
...
. Create a directory that will survive workspace restarts:
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
...
----
+
. Create the following script for the purpose of executing it when your workspace restarts:
+
[source,bash,role=execute,subs="verbatim,attributes",title="/projects/env/config.sh"]
----
#!/usr/bin/env bash
...
----
+
. Now if your workspace restarts, simply execute this script into the environment:
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
...
----
+
. Add these commands to the `/projects/env/config.sh` script

=== 1.3: Branches

link:https://git-scm.com/book/en/v2/Git-Branching-Branches-in-a-Nutshell[Git branches,window=_blank] should be used liberally. They can be used to test ideas and have no fear trashing a branch that didn't quite work out. It's better to leave a branch as is and start a new one based on your current working branch rather than testing a few things and reverting the commits or to comment/uncomment things. Using the new `git` prompt configured previously, your prompt should update when your branch changes similar to the following:

. Execute this lab in the `/projects/ansible-devspaces`
+
[source,role=execute,bash,subs="verbatim,attributes"]
----
cd /projects/ansible-devspaces
----
. Check what branch you are in. Notice it is also dynamically displayed in your prompt:
+
[source,role=execute,bash,subs="verbatim,attributes"]
----
git branch
...
* ansible-superlab
  main
[user ansible-devspaces (ansible-superlab)]$ 
----
+
. Create a new branch called `test`, which is a copy of `main` using `git switch`:
+
NOTE: Historically, this was accomplished via `git checkout -b <branch_name>` but the `switch` sub-command was introduced in version `2.23` for this purpose.
+
[source,role=execute,bash,subs="verbatim,attributes"]
----
...
[user ansible-devspaces (test)]$ 
----
<1>  `ansible-devspaces` is the name of the branch where contents associated with this lab are located.
<2> `main` is the name of the branch that is typically used as the default branch. Historically `master` was used in the past, however, most newer repositories make use of `main` as the default branch instead. The `git config --global init.defaultBranch` command can be used to configure your `git` client with the name of the default branch for new repositories.
<3> The asterisk shows the currently active branch.
+
...
+
. Switch back to the `ansible-devspaces` branch:
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
git switch ansible-superlab
----
+
[source,output]
----
Switched to branch 'ansible-superlab'
Your branch is up to date with 'origin/ansible-superlab'.
[user ansible-devspaces (ansible-superlab)]$ 
----
+
...
----
+
. Gitleaks is available as a container image, so no installation is necessary. Pull the image into your Dev Spaces workspace:
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
podman run -v /projects/ansible-devspaces/:/pwd:Z zricethezav/gitleaks:latest dir -v /pwd/
----
+
[source,output]
----
...
+
NOTE: If the UUID does not trigger this scan, try re-generating a new one.
...
Collecting pre-commit
  Obtaining dependency information for pre-commit from https://files.pythonhosted.org/packages/80/6e/4b28b62ecb6aae56769c34a8ff1d661473ec1e9519e2d5f8b2c150086b26/pre_commit-4.6.0-py2.py3-none-any.whl.metadata
  Downloading pre_commit-4.6.0-py2.py3-none-any.whl.metadata (1.2 kB)
Collecting cfgv>=2.0.0 (from pre-commit)
  Obtaining dependency information for cfgv>=2.0.0 from https://files.pythonhosted.org/packages/db/3c/33bac158f8ab7f89b2e59426d5fe2e4f63f7ed25df84c036890172b412b5/cfgv-3.5.0-py2.py3-none-any.whl.metadata
  Downloading cfgv-3.5.0-py2.py3-none-any.whl.metadata (8.9 kB)
Collecting identify>=1.0.0 (from pre-commit)
  Obtaining dependency information for identify>=1.0.0 from https://files.pythonhosted.org/packages/94/84/d9273cd09688070a6523c4aee4663a8538721b2b755c4962aafae0011e72/identify-2.6.19-py2.py3-none-any.whl.metadata
  Downloading identify-2.6.19-py2.py3-none-any.whl.metadata (4.4 kB)
Collecting nodeenv>=0.11.1 (from pre-commit)
  Obtaining dependency information for nodeenv>=0.11.1 from https://files.pythonhosted.org/packages/88/b2/d0896bdcdc8d28a7fc5717c305f1a861c26e18c05047949fb371034d98bd/nodeenv-1.10.0-py2.py3-none-any.whl.metadata
  Downloading nodeenv-1.10.0-py2.py3-none-any.whl.metadata (24 kB)
Requirement already satisfied: pyyaml>=5.1 in /usr/lib64/python3.12/site-packages (from pre-commit) (6.0.1)
Requirement already satisfied: virtualenv>=20.10.0 in /usr/local/lib/python3.12/site-packages (from pre-commit) (21.3.0)
Requirement already satisfied: distlib<1,>=0.3.7 in /usr/local/lib/python3.12/site-packages (from virtualenv>=20.10.0->pre-commit) (0.4.0)
Requirement already satisfied: filelock<4,>=3.24.2 in /usr/local/lib/python3.12/site-packages (from virtualenv>=20.10.0->pre-commit) (3.29.0)
Requirement already satisfied: platformdirs<5,>=3.9.1 in /usr/local/lib/python3.12/site-packages (from virtualenv>=20.10.0->pre-commit) (4.9.6)
Requirement already satisfied: python-discovery>=1.2.2 in /usr/local/lib/python3.12/site-packages (from virtualenv>=20.10.0->pre-commit) (1.2.2)
Downloading pre_commit-4.6.0-py2.py3-none-any.whl (226 kB)
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 226.5/226.5 kB 11.7 MB/s eta 0:00:00
...
Successfully installed cfgv-3.5.0 identify-2.6.19 nodeenv-1.10.0 pre-commit-4.6.0
----
+
NOTE: Notice the *WARNING:* this tells you the path the binary was installed to.
+
. Create a _pre-commit_ configuration file by creating a new file called `.pre-commit-config.yaml` with the following content:
+
[source,yaml,title="/projects/ansible-devspaces/.pre-commit-config.yaml",role=execute,subs="verbatim,attributes"]
----
repos:
  - repo: https://github.com/gitleaks/gitleaks
    ...
    hooks:
      - id: gitleaks
        name: gitleaks
        entry: podman run -v /projects/ansible-devspaces:/scan:Z zricethezav/gitleaks:latest dir -v /scan/
        language: system
        stages: [pre-commit]
----
+
...
----
+
. In this way, a developer never needs to remember to run a tool before a commit. _pre-commit_ will do the work for you, keeping mistakes out of git history.
. Now, unstage the file so it will not be committed:
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
...
For a more comprehensive exploration of pre-commit and the pre-commit-config, see link:https://github.com/redhat-cop/infra.aap_configuration/blob/devel/.pre-commit-config.yaml[infra.aap_configuration pre-commit config^]
...
Ansible includes a Visual Studio Code (VS Code) extension to enhance the development experience. You can learn more about the features that are available in the extension at link:https://ansible.readthedocs.io/projects/vscode-ansible/[Ansible VS Code Extension documentation^]
...
+
image::12-platform-dev-experience/ansible-extension1.png[Ansible extension panel in VS Code sidebar,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-creator1.png[Ansible Creator playbook project creation dialog with destination directory and namespace fields,link=self,window=blank]
+
...
. Click **Create**
+
image::12-platform-dev-experience/ansible-creator2.png[Ansible Creator project creation log output,link=self,window=blank]
+
. The logs will display in the box below that
. Click `Open Project` to open a new VS Code window with this new project
+
image::12-platform-dev-experience/ansible-creator3.png[Open Project button in Ansible Creator after successful creation,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-creator4.png[Scaffolded project directory structure in VS Code explorer,link=self,window=blank]
+
. The `inventory` director contains a ready to use directory structure:
+
image::12-platform-dev-experience/ansible-creator6.png[Inventory directory structure created by ansible-creator,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-creator5.png[Collections directory structure with mycollection and run role,link=self,window=blank]
...
. Close the *collections* folder if you expanded it during the previous steps
. Scroll down to the very last file to the bottom left called `site.yml`
. Right-click on `site.yml` and choose `Run Ansible Playbook Via ...`
. Choose `Run playbook via  'ansible-playbook'`.
+
image::12-platform-dev-experience/ansible-playbook1.png[VS Code context menu showing Run Ansible Playbook Via ansible-playbook option,link=self,window=blank]
+
. A terminal opens up with the results of the `ansible-playbook` execution:
+
image::12-platform-dev-experience/ansible-playbook2.png[Terminal output showing ansible-playbook execution results,link=self,window=blank]
...
+
image::12-platform-dev-experience/ansible-navigator1.png[VS Code context menu showing Run Ansible Playbook Via ansible-navigator run option,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-navigator2.png[ansible-navigator TUI showing Complete status after playbook run,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-navigator3.png[ansible-navigator task inspection view showing play and task details,link=self,window=blank]
+
. Once reviewed, hit `ESC` several times until you exit `ansible-navigator` and return back to the terminal.
+
image::12-platform-dev-experience/ansible-navigator4.png[Terminal prompt after exiting ansible-navigator,link=self,window=blank]
...
. Append to the existing `site.yml` playbook with the following poorly written task:
+
...
+
image::12-platform-dev-experience/ansible-lint1.png[VS Code status bar showing ansible-lint running after file save,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-lint2.png[VS Code editor showing ansible-lint violations with red squiggly underlines on debug task,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-lint3.png[Ansible extension gear icon for opening settings,link=self,window=blank]
+
. Open **Settings** for the Ansible extension:
+
image::12-platform-dev-experience/ansible-lint4.png[Ansible extension settings panel in VS Code,link=self,window=blank]
+
. Under the Ansible extension settings, click on the **Validation** subsection
. Select the checkbox for `Auto Fix on Save`.
+
image::12-platform-dev-experience/ansible-lint5.png[Ansible Validation settings with Auto Fix on Save checkbox,link=self,window=blank]
+
...
+
image::12-platform-dev-experience/ansible-lint6.png[VS Code editor showing ansible-lint auto-fix results after save,link=self,window=blank]
+
NOTE: If it it does not automatically fix these errors, try closing and reopening the file.
+
...
. https://github.com/redhat-cop/infra.aap_configuration/blob/devel/.pre-commit-config.yaml[infra.aap_configuration pre-commit config^]
. https://ansible.readthedocs.io/projects/dev-tools/[Ansible Development Tools^]
. https://ansible.readthedocs.io/projects/vscode-ansible/[Ansible VS Code Extension^]
