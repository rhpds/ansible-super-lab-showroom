...
== 1. Disable check_website Rulebook Activation

Before starting this lab, disable the `check_website` Rulebook Activation created in the previous lab so that it does not interfere with this lab.
...
+
image::31-events-log-streaming/eda-ra-overview.png[EDA Rulebook Activations Overview]
. Deselect the toggle for the `check_website` Rulebook Activation to disable it
+
image::31-events-log-streaming/eda-ra-disable-url-check.png[Disabled EDA Rulebook Activation]
+
. Select the checkbox to confirm disabling and click **Disable rulebook activations**
+
image::31-events-log-streaming/eda-ra-disable-confirm.png[Confirm Disable EDA Rulebook Activation]

== 2: Create the Configuration Playbook

In the previous module, we built and ran a automation that installed `Chocolatey` and `Filebeat` to the Windows host. In this lab, we will add additional configurations for Filebeat.

. In your `lab_playbooks` repository, create the `setup_monitoring.yml` playbook:
+
...
...
----
<1> For debugging, this property can be set to `true` to provide greater readability in the logs.
+
...
----
+
. Add, commit and push the files to the `lab_playbooks` git repository
+
[source,bash,role=execute,subs="verbatim,attributes"]
----
...
. In your `lab_playbooks_ repository, create the `check_kafka.yml` rulebook in the `rulebooks` directory:
+
...
----
+
. Add, commit and push this rulebook to the `lab_playbooks` git repository
+
...
. Click the Sync button next to the `Lab Playbooks` project
+
image::31-events-log-streaming/eda-projects-sync.png[Sync EDA Project]


=== 5: Create the Rulebook Activation
...
+
image::31-events-log-streaming/eda-ra-create.png[Create Rulebook Activation]

== 6: Simulate Failure

Return to OpenShift Dev Spaces and intentionally stop the service to verify the end-to-end flow. After the service is stopped, attempt to access the IIS web server to confirm it is not responding.
...
. Find and *click* on the `check_kafka` Rulebook Activation
. Select the *History* tab for the `check_kafka` Rulebook Activation
. Check the **EDA Controller** Rulebook Activation logs for the Kafka event.
. Confirm the `Start IIS` was triggered in **Automation Controller**.
+
[source,output]
----
...
ansible_rulebook.action.run_job_template - INFO - running job template: Start IIS, organization: Default
...
. Go to the link:{openshift_cluster_console_url}[OpenShift console,window=_blank]
...
. https://console.redhat.com/ansible/automation-hub/repo/published/ansible/eda/content/eda%2Fplugins%2Fevent_source/kafka/[Official kafka documentation]
