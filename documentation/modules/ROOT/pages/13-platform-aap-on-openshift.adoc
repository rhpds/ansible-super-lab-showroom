...
More information can be found on the official link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.7/install-assembly_operator_install_operator[AAP on OpenShift documentation,window=_blank].
...
AAP has been installed in the `aap` Namespace using an link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html-single/operators/index[Operator,window=_blank] and the Operator Lifecycle Manager (OLM). The operator manages several Custom Resource Definitions (CRDs) that define the different components of Ansible Automation Platform.
...
+
image::13-platform-aap-on-openshift/installed-operators-aap.png[Installed Operators in the AAP Project,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-operator-all-instances.png[All Instances of the AAP Operator,link=self,window=blank]
+
...
image::13-platform-aap-on-openshift/aap-operator-aap-details.png[AAP Instance Details,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-deployments.png[AAP Deployments,link=self,window=blank]
+
...
image::13-platform-aap-on-openshift/aap-deployment-details.png[AAP Deployment Details,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-pods.png[AAP Pods,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-pod-details.png[AAP Pod Details,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-pod-logs-task.png[AAP Pod Logs - Task Container,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-pod-logs-rsyslog.png[AAP Pod Logs - Rsyslog Container,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-pvcs.png[AAP Persistent Volume Claims,link=self,window=blank]
+
...
image::13-platform-aap-on-openshift/aap-pvc-details.png[AAP Persistent Volume Claim Details,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-configmaps.png[AAP ConfigMaps,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-configmap-details.png[AAP ConfigMap Details,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-secrets.png[AAP Secrets,link=self,window=blank]
+
* Click on **aap-admin-password**. Under `Details` we can see information about the secret and its data.
+
image::13-platform-aap-on-openshift/aap-secret-details.png[AAP Secrets,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-routes.png[AAP Routes,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-route-details.png[AAP Route Details,link=self,window=blank]
+
...
image::13-platform-aap-on-openshift/aap-project.png[AAP Project Details,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-operatorhub.png[AAP OperatorHub,link=self,window=blank]
+
...
. Underneath _Channel_ select **stable-2.7**.
. Click the **Install** button.
+
image::13-platform-aap-on-openshift/aap-operator-install.png[AAP Operator Install,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-operator-install-2.png[AAP Operator Install - Namespace Selection,link=self,window=blank]
+
. Click the **Install** button.
+
image::13-platform-aap-on-openshift/aap-operator-install-progress.png[AAP Operator Install - Namespace Selection,link=self,window=blank]
+
...
+
Under the list of installed operators you should now see `Ansible Automation Platform` with a version of `2.7.+0.1...` and a status of `Successful`.

image::13-platform-aap-on-openshift/aap-operator-install-success.png[AAP Operator Install Success,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-operator-secret-type.png[AAP Operator Secret Type Selection,link=self,window=blank]
+
...
image::13-platform-aap-on-openshift/aap-operator-secret.png[AAP Operator Secret,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-operator-create-new.png[Create New AAP Instance,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-operator-resources.png[AAP Resources Tab,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-running.png[AAP Running Status,link=self,window=blank]
...
+
NOTE: You may need to create a manifest using your account, refer to link:https://access.redhat.com/articles/5807761[kbase 5807761,window=_blank]
+
...
image::13-platform-aap-on-openshift/aap-automationhub-status.png[AutomationHub Status,link=self,window=blank]
...
The operator deployed AAP instance is pre-configured with a default link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.7/administer-con_controller_container_groups[Container Group,window=_blank]. A container group is an instance group that points to an OpenShift cluster. In this case, the OpenShift cluster that the operator was installed on.
...
+
image::13-platform-aap-on-openshift/aap-settings-troubleshooting.png[AAP Automation Execution Settings,link=self,window=blank]
+
. Click on the **Edit** button.
. Uncheck the **Release Receptor Work** checkbox.
. Click the **Save** button.

image::13-platform-aap-on-openshift/aap-receptor-work.png[AAP Receptor Work Setting,link=self,window=blank]
...
+
image::13-platform-aap-on-openshift/aap-instance-groups.png[AAP Instance Groups,link=self,window=blank]
+
...
+
image::13-platform-aap-on-openshift/aap-edit-instance-group.png[Update Default Instance Group,link=self,window=blank]
+
...
. Navigate to **Workloads** on the left hand navigation -> **Pods**.
* Next to the `Project:` dropdown in the top left, ensure `my-aap` is the project shown.
. In the filter dropdown, select **Name**
. In the filter text box, enter `automation-job`.
. Click on the `automation-job-<id>-<guid>` pod.
+
image::13-platform-aap-on-openshift/aap-job-pods.png[Job Pods,link=self,window=blank]
+
...
image::13-platform-aap-on-openshift/aap-container-group-pod.png[AAP Container Group Pod Labels,link=self,window=blank]
...
Now that you've had some experience deploying and managing AAP on OpenShift using the AAP operator, let's go ahead and uninstall the operator and the deployed instance. This process will not only enable you with the understanding of how to uninstall the operator and its managed resources, but also free up cluster resources.

=== 6.1: Uninstall the AAP Instance

Remove the `aap` instance of the `AnsibleAutomationPlatform` custom resource from the `my-aap` namespace.

CAUTION: It is important that only the resources in the `my-aap` namespace are to be removed. Modifying the resources in the `aap` could affect te remainder of the lab.

. Navigate to **Ecosystem** on the left hand navigation -> **Installed Operators**.
. Next to the `Project:` dropdown in the top left, ensure `my-aap` is the project shown.
. Click on the `Ansible Automation Platform` operator.
. In the toolbar click on `All instances`.
. Next to the `aap` instance, click on the kebab (three dots) and select  **Delete AnsibleAutomationPlatform**.
+
image::13-platform-aap-on-openshift/delete-aap-instance1.png[Delete the AAP Instance,link=self,window=blank]
+
. Click the **Delete** button on the confirmation dialog.
+
image::13-platform-aap-on-openshift/delete-aap-instance2.png[Delete the AAP Instance Confirmation Dialog,link=self,window=blank]

The AAP operator will now recognize that the `AnsibleAutomationPlatform` instance has been removed and will proceed to remove all the resources that were created as part of this instance. This may take several minutes to complete for the entire process to complete.

[#uninstall-aap-instance]
=== 6.2 Uninstall the AAP Operator

Now that the AAP instance is removed, uninstall the AAP operator from the `my-aap` namespace.

. Navigate to **Ecosystem** on the left hand navigation -> **Installed Operators**.
. Next to the `Project:` dropdown in the top left, ensure `my-aap` is the project shown.
. Click on the kebab (three dots) next to the `Ansible Automation Platform` operator and select **Uninstall Operator**.
+
image::13-platform-aap-on-openshift/uninstall-aap-operator.png[Uninstall the AAP Operator,link=self,window=blank]
+
. Click the **Uninstall** button on the confirmation dialog to remove the AAP operator.

=== 6.3 Remove the my-aap Project

Finally, remove the `my-aap` project that was created for this exercise.

. Navigate to **Home** on the left hand navigation -> **Projects**.
. Click on the kebab (three dots) next to the `my-aap` project and select **Delete Project**.
+
image::13-platform-aap-on-openshift/delete-my-aap-project.png[Delete the my-aap Project,link=self,window=blank]
+
. Click the **Delete** button on the confirmation dialog to remove the `my-aap` project
...
This knowledge prepares you to deploy, manage, and troubleshoot AAP on OpenShift in enterprise environments, using Kubernetes-native automation platform management.
