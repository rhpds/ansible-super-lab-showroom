...
. Platform
[arabic]
.. xref:11-platform-workspace.adoc[Introduction and exploration of Dev Spaces Workspace]
.. xref:12-platform-dev-tooling.adoc[Using Ansible developer tools]
.. xref:13-platform-aap_on_openshift.adoc[AAP on OpenShift]
. Content
[arabic]
.. xref:21-content-develop.adoc[Creating automation for Windows]
.. xref:22-content-build.adoc[Build a custom Execution Environment and Collection]
.. xref:23-content-execute.adoc[Create a Playbook and configure AAP to execute it all]
.. xref:24-content-ansible-tdd.adoc[TDD with Molecule]
.. xref:25-content-config-as-code.adoc[Config as Code]
.. xref:26-content-pipeline.adoc[Pipeline for collections]
. Events
[arabic]
.. xref:31-events-health-probe.adoc[EDA Overview and Rulebook for health monitoring]
.. xref:32-events-log-streaming.adoc[EDA Log streaming with Filebeat and Kafka]
.. xref:33-events-troubleshooting.adoc[EDA Troubleshooting]
. AI
[arabic]
.. xref:41-ai-lightspeed-int-assistant.adoc[Ansible Lightspeed Intelligent Assistant]
.. xref:42-ai-lightspeed-mcp.adoc[Ansible MCP Server]
...
Begin with xref:11-platform-workspace.adoc[Module 1 Lab 1: Platform - Introduction and exploration of Dev Spaces Workspace] to set up your development environment. Each module includes detailed instructions, code examples, and verification steps.
