...
** Content from xref:21-content-develop.adoc[Lab 2.1 Creating automation for Windows]
...
. From the navigation menu on the left, expand **Access Management** and click on **API Tokens**.
+
image::22-content-build/api-token-overview.png[API Tokens Overview,link=self,window=blank]
+
. Click the **Create API token** button
+
image::22-content-build/api-token-create.png[API Tokens Overview,link=self,window=blank]
+
. On the Create API Token page, enter `automation-hub` in _description_ field, ensure the _Scope_ dropdown is set as **Write** and click the **Create token** button. Your API token will be displayed.
+
image::22-content-build/api-token-generate.png[Generate an API Token,link=self,window=blank]
+
...
----
+
. Also create a corresponding file named `unset_pah_vars.env` in the `/projects/env/` directory to clear these variables for any troubleshooting purposes:
+
...
. Update the `galaxy.yml` file and modify the `dependencies` section to appear as follows (ensuring `ansible.utils` is removed):
+
...
+
image::22-content-build/navigate-namespaces.png[Namespaces navigation,link=self,window=blank]
+
...
+
image::22-content-build/create-namespace.png[Namespace creation form,link=self,window=blank]
...
+
image::22-content-build/collections-approvals-pending.png[Approve the Collection,link=self,window=blank]
+
...
+
image::22-content-build/collections-approvals-selection.png[Select Collection to Approve,link=self,window=blank]
+
. Select the _Yes, I confirm that I want to approve these 1 collections._ checkbox and click **Approve collections** button.
+
image::22-content-build/collections-approvals-approve-collection.png[Pending Approval Collection,link=self,window=blank]
+
...
+
image::22-content-build/pah-collections.png[PAH Collections,link=self,window=blank]
...
+
image::22-content-build/create-remote-registry.png[Create Remote Registry,link=self,window=blank]
+
...
+
image::22-content-build/create-remote-registry-details.png[Create Remote Registry Details,link=self,window=blank]
+
...
+
image::22-content-build/create-execution-environment.png[Create Execution Environment,link=self,window=blank]
+
. Enter the following details on the _Create Execution Environment_ page:
.. Name: `ansible-automation-platform-27/ee-minimal-rhel9`
.. Upstream Name: `ansible-automation-platform-27/ee-minimal-rhel9`
.. Remote Registry: Use the drop-down to select `redhat` that was just created.
.. To reduce the sync time and storage requirements, enter `2.16` in the _Add tag(s) to include_ field and click **Add**
.. Click the **Create execution environment** button to create the new EE.
+
image::22-content-build/create-execution-environment-form.png[Create Execution Environments Form,link=self,window=blank]
+
. Synchronize the Execution Environment into PAH from the Red Hat Container Registry.
. Next to the `ansible-automation-platform-27/ee-minimal-rhel9` Execution Environment, click the kebab menu (three vertical dots) on the right hand side of the entry and select **Sync execution environment**.
+
image::22-content-build/execution-environment-sync.png[Sync Execution Environment,link=self,window=blank]
+
...
+
image::22-content-build/execution-environment-confirm-sync.png[Confirm Sync Execution Environment,link=self,window=blank]
+
...
+
NOTE: The sync process may take several minutes to complete and will show a status of _Pending_ until the sync is complete.
...
+
image::22-content-build/edit-certified-remote.png[Edit RH-Certified Repository,link=self,window=blank]
+
...
. Click *Save remote*
+
image::22-content-build/edit-certified-details.png[Edit Details,link=self,window=blank]
+
. Navigate to *Automation Content* -> *Repositories*, next to *rh-certified* click the kebob (3 dots) and select *Sync repository*
+
image::22-content-build/sync-certified-repository.png[Sync Repository,link=self,window=blank]
+
. In the final dialog, leave the default values and click **Sync**.
+
image::22-content-build/sync-repo-confirm.png[caption="Confirm Sync", role="thumb",link=self,window=blank]
...
+ 
image::22-content-build/gitea-homepage.png[Gitea Homepages,link=self,window=blank]
+
...
+ 
image::22-content-build/gitea-login.png[Gitea Login Page,link=self,window=blank]
+
. Once authenticated, in the top left of the web interface, click on the **+** symbol and select **New Repository**.
+
image::22-content-build/gitea-navigate-new-repository.png[Navigate to new Gitea Repository,link=self,window=blank]
+
. On the *New Repository* page, enter `red_hat_one_super_lab` in the *Repository Name* field.
+ 
image::22-content-build/gitea-new-repository-name.png[Gitea New Repository,link=self,window=blank]
+
. Leave everything else as default and click on the button at the bottom, **Create Repository**.

image::22-content-build/gitea-create-repository.png[Gitea Create Repository,link=self,window=blank]
...
git remote add origin \
  {gitea_console_url}/{gitea_user}/red_hat_one_super_lab.git
----
+
...
+
image::22-content-build/devspaces-git-auth.png[Gitea Repository Containing Collection Content,link=self,window=blank]
+
...
+
image::22-content-build/gitea-repository.png[Dev Spaces Git Authentication,link=self,window=blank]
...
images:
  base_image:
    name: aap-aap.{openshift_cluster_ingress_domain}/ansible-automation-platform-26/ee-minimal-rhel9:2.16
...
+
image::22-content-build/collection-uploaded.png[Confirm Collection Upload,link=self,window=blank]
...
additional_build_steps:
  prepend_galaxy:
    ...
    - ENV ANSIBLE_GALAXY_SERVER_PUBLISHED_TOKEN=$\{TOKEN\}
----
* Collection version `1.0.1` increments the collection version.
* The `python` section is added to include the `cowsay` dependency. Ensure this lines up properly with `galaxy` and NOT `collections`.
...
+
image::22-content-build/collection-verify-new-version.png[Verify New Collection Version,link=self,window=blank]
...
+
image::22-content-build/execution-environment-versions.png[View EE Versions,link=self,window=blank]
