= Lab: Develop a Playbook and Execute within AAP
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
ansible-navigator --execution-environment-image aap-aap.{openshift_cluster_ingress_domain}/my-pah-ee:1.1 run test_pah_ee.yml  --mode stdout
----
+
[source,output]
----
...
+
image::22-content-execute/gitea-lab-playbooks.png[Lab Playbooks]
...
+
image::22-content-execute/automation-controller-credentials-overview.png[Navigate to Credentials]
+
. Click **Create credential** to add a new credential.
+
image::22-content-execute/automation-controller-create-credential.png[Create Credentials]
+
...
image::22-content-execute/automation-controller-create-credential-dialog.png[Create Credentials Dialog]
...
+
image::22-content-execute/automation-controller-create-credential-2.png[Create Credentials]
+
. Enter the following details on the _Create Credential_ page:
.. *Name*: `Gitea`
...
. Click **Create credential** to save the new credential.
+
image::22-content-execute/automation-controller-create-credential-git-dialog.png[Create Credentials Dialog]
...
+
image::22-content-execute/automation-controller-ee-overview.png[Navigate to Execution Environments]
+
. Click **Create execution environment** to add a new execution environment.
+
image::22-content-execute/automation-controller-create-ee.png[Create Execution Environment]
+
...
image::22-content-execute/automation-controller-create-ee-dialog.png[Create Execution Environment Dialog]

=== 2.5: Create PAH Credentials

In order for Automation Controller to pull collections from Private Automation Hub, we need to a Credential for each of the repositories containing collections we wish to use in our automation.

. From the Credentials page in Automation Controller, click **Create credential** once again to add a new credential.
+
image::22-content-execute/automation-controller-create-credential-3.png[Create Credentials]
+
. Enter the following details on the _Create Credential_ page to create a credential for the _published_ repository in Private Automation Hub. Use the API token obtained in the prior lab:
.. *Name*: `PAH - published`
...
. Click **Create credential** to save the new credential.
+
image::22-content-execute/automation-controller-create-credential-pah-dialog.png[Create Credentials Dialog]
...
+
image::22-content-execute/automation-controller-organizations-overview.png[Navigate to Organizations]
+
. Next to the _Default_ organization, click the pencil icon to edit the organization.
+
image::22-content-execute/automation-controller-organizations-edit.png[Edit Organization]
+
...
+
image::22-content-execute/automation-controller-organizations-add-credentials.png[Add Credentials to Organization]
+
...
+
image::22-content-execute/automation-controller-projects-overview.png[Navigate to Projects]
+
. Click **Create project** to add a new project.
+
image::22-content-execute/automation-controller-create-project.png[Create Project]
+
...
+
image::22-content-execute/automation-controller-create-project-dialog.png[Create Project Dialog]
+
. Confirm that the project has been synced successfully by looking _Success_ underneath _Last job status_.
+
image::22-content-execute/automation-controller-sync-project.png[Project Sync Status]
...
+
image::22-content-execute/automation-controller-templates-overview.png[Navigate to Templates]
+
. Click the **Create template** dropdown and select **Create job template**.
+
image::22-content-execute/automation-controller-create-job-template.png[Create Job Template]
+
...
+
image::22-content-execute/automation-controller-create-job-template-dialog.png[Create Job Template Dialog]
...
+
image::22-content-execute/automation-controller-launch-job-template.png[Launch Job Template]
+
...
+
image::22-content-execute/automation-controller-job-template-success.png[Job Template Success]

=== 2.8: Add Job Template for Windows Node

Repeat these steps to add a *Job Template* to execute the playbook we created in xref:20-content-windows.adoc[Module 2 Lab 1,window=_blank] page.
...
----
+
image::22-content-execute/jt-provision-windows.png[Job Template Success]
...
+
image::22-content-execute/remotes-overview.png[Remotes Navigation]
+
. Click the pencil icon to edit the `community` remote.
+
image::22-content-execute/remotes-edit-community.png[Edit Community Remote]
+
...
=== 3.2: Add Validated Remote

Create a new remote for the `validated` collections repository. The API token for upstream Red Hat Automation Hub is required to access this repository. The token retrieved in a prior lab can be used here.

. From the Remotes page in Private Automation Hub, click **Create remote**.
+
image::22-content-execute/remotes-create-remote.png[Create Remote]
+
...
image::22-content-execute/remotes-add-remote.png[Add Remote]
...
+
image::22-content-execute/repositories-overview.png[Add Remote]
+
. Edit the _validated_ repository by selecting the pencil icon
+
image::22-content-execute/repositories-edit-validated.png[Edit Validated Repository]
+
. Update the `Remote` field by selecting `validated` from the drop down.
. Click **Save repository** to update the repository.

image::22-content-execute/repositories-update-validated.png[Updated Validated Repository]

=== 3.3: Sync Repositories
...
+
image::22-content-execute/sync-repository.png[Sync Repository]
+
. In the _Sync Repository_ dialog, click **Sync** to start the synchronization.
. Repeat the above steps to sync the `validated` repositories as well.
