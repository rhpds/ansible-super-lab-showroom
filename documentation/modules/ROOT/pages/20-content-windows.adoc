...
[source,bash]
----
for role in web_server log_shippper
  do 
...
. Create a new task file in the `tasks` directory for the `web_server` role pre-requisites:
+
[source,yaml, title="/projects/lab_collection/roles/web_server/tasks/prerequisites.yml"]
----
---
...
----
+
. Create another new task file in that same `tasks` directory for the installation tasks:
+
[source,yaml, title="/projects/lab_collection/roles/web_server/tasks/web_server.yml"]
----
---
...
+
[source,bash, title="/projects/lab_collection/roles/web_server/files/index.html.j2"]
----
...
+
[source,yaml, title="/projects/lab_collection/roles/web_server/tasks/main.yml"]
----
---
...
+
[source,yaml, title="/projects/lab_collection/roles/log_shipper/tasks/setup.yml"]
----
---
...
+
[source,yaml, title="/projects/lab_collection/roles/log_shipper/tasks/install.yml"]
----
---
...
+
[source,yaml, title="/projects/lab_collection/roles/log_shipper/tasks/main.yml"]
----
---
...
. Remove the entry in `/projects/lab_playbooks/inventory/group_vars/all.yml` for `ansible_user`
+
. Modify the main inventory file and replace it with these lab details:
+
[source,yaml, title="/projects/lab_playbooks/inventory/hosts.yml"]
----
---
all:
  children:
...
+
[source,yaml, title="/home/user/secrets.yml"]
----
---
...
+
[source,bash, title="/projects/lab_playbooks/ansible.cfg"]
----
[galaxy]
...
+
[source,yaml, title="/projects/lab_playbooks/collections/requirements.yml"]
----
---
collections:
...
This playbook scaffold required a collection name when executing. The reason is `ansible-creator` creates a collection and single role for use by this playbook. Since we already have our own separate collection, we will remove the provided collection and link it to `/projects/lab_collection/`.
...
+
[source,yaml, title="/projects/lab_playbooks/site.yml"]
----
---
...
. From `/projects/lab_playbooks/` run:
+
[source,bash]
----
ansible-playbook site.yml -e @/home/user/secrets.yml
----
+
...
. Using `ansible-creator` to scaffold the collection and roles associated with automation targeting a Windows instance
