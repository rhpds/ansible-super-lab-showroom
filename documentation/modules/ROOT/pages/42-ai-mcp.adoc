...
* **Prepares you for:** Using Ansible MCP Server
...
+
image::42-ai-mcp/installed-operators-aap.png[Installed Operators in the aap Project,link=self,window=blank]
+
...
+
image::42-ai-mcp/ansible-automation-platform-instances.png[Ansible Automation Platform Instances in the aap Project,link=self,window=blank]
+
...
+
image::42-ai-mcp/api-token-overview.png[API Token Overview,link=self,window=blank]
+
...
+
image::42-ai-mcp/api-token-details.png[API Token Creation Details,link=self,window=blank]
+
...
+
image::42-ai-mcp/openshift-overview-import-yaml.png[Import YAML option in OpenShift console,link=self,window=blank]
...
+
image::42-ai-mcp/dev_spaces_shortcut.png[OpenShift Dev Spaces Shortcut Icon,link=self,window=blank]
+
...
+
image::42-ai-mcp/restart-devspaces-workspace.png[Restart Dev Spaces Workspace option,link=self,window=blank]
+
...
image::42-ai-mcp/opencode-overview.png[OpenCode Overview,link=self,window=blank]
...
image::42-ai-mcp/opencode-status.png[OpenCode Status,link=self,window=blank]
...
image::42-ai-mcp/opencode-mcp-invoke.png[OpenCode MCP Tool Invocation,link=self,window=blank]
...
+
image::42-ai-mcp/librechat-edit-secret.png[Edit the librechat-credentials-env Secret,link=self,window=blank]
+
...
+
image::42-ai-mcp/librechat-edit-secret-value.png[Edit ANSIBLE_MCP_TOKEN in the librechat-credentials-env Secret,link=self,window=blank]
+
...
+
image::42-ai-mcp/librechat-pods-filter.png[Filter the librechat Pods by Label,link=self,window=blank]
+
...
+
image::42-ai-mcp/librechat-delete-pod.png[Delete the librechat Pod,link=self,window=blank]
+
...
+
image::42-ai-mcp/librechat-login-page.png[LibreChat Login Page,link=self,window=blank]
+
...
+
image::42-ai-mcp/librechat-enable-model.png[Enable the {litellm_available_models_list} Model in LibreChat,link=self,window=blank]
+
...
+
image::42-ai-mcp/librechat-enable-mcp.png[Enable aap MCP Server in LibreChat,link=self,window=blank]
+
Now, lets start a chat session using our natural language that retrieves details that originate from Ansible Automation Platform that is facilitated through the integration with the AAP MCP Server.
+
...
image::42-ai-mcp/librechat-chat-result.png[LibreChat Chat Result,link=self,window=blank]
