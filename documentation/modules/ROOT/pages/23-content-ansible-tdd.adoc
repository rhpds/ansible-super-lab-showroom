...
Now, move the Molecule playbooks into the `utils` directory for sharing between scenarios.
...
The primary logic behind Molecule execution is defined in the link:https://ansible.readthedocs.io/projects/molecule/configuration/[`molecule.yml`,window=_blank] configuration file. Replace the contents of `molecule/db_server/molecule.yml` with the following configuration:

[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/db_server/molecule.yml"]
----
---
dependency:
  name: galaxy
...
Create a new `requirements.yml` file in the `molecule/db_server/` directory to specify the required Ansible collections for this scenario:
...
The scenario uses shared playbook files for container management and testing. Create the following files in the `molecule/utils/playbooks/` directory:
...
[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/create.yml"]
----
---
...
Create a new directory called `tasks` inside `molecule/utils/playbooks/` and add the `create-fail.yml` playbook. This playbook handles failure scenarios during container creation by retrieving and displaying container logs for debugging.
...
The `prepare.yml` playbook handles any pre-testing setup tasks. This optional phase in Molecule allows you to configure the test environment before applying your Ansible role, such as installing dependencies or setting up prerequisites. Create a new file called `prepare.yml` in the `molecule/utils/playbooks/` directory with the following content:

[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/prepare.yml"]
----
---
- name: Prepare play
  hosts: molecule
...
[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/converge.yml"]
----
---
- name: Converge
  hosts: all
...
[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/verify.yml"]
----
---
- name: Verify
  hosts: all
...
The `cleanup.yml` playbook handles cleanup of temporary files and artifacts created during testing, helping maintain a clean test environment between test runs without destroying the actual test infrastructure. Create a new file called `cleanup.yml` in the `molecule/utils/playbooks/` directory with the following content:

[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/cleanup.yml"]
----
---
...
[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/destroy.yml"]
----
---
...
[source,yaml,role=execute,subs="verbatim,attributes",title="molecule/utils/playbooks/noop.yml"]
----
---
- name: No-op
  hosts: localhost
